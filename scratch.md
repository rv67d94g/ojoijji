...
规范是十分重要的。国际标准化组织（ISO）就是专门为此设立的机构。他成立于1947年2月23日，是制定全世界工商业国际标准的国际标准建立机构。几乎所有国家都是他的会员团体。有了共同的标准，不同的计算平台之间也能够互相交换信息。他也能够保护用户避免厂商锁定（vendor lock-in）。规范在办公文档之中尤为重要，因为现在的人依赖于过去的文档，并且要保证将来的人也能打开现在的文档。

不过微软企图危害这一重要利益。他企图用专门为MS Office系列软件设计的文档格式（如docx），代替自由的文档格式。他动用自己的游说力量说服了ISO组织，同时也损害了这个重要组织的信誉。

开放文档格式（OpenDocument Format）是一种规范，基于XML的文件格式，因应表格、图表、演示稿和文字处理文件等电子文件而设置。他对各种不同的平台有良好的支持。而微软的Office Open XML（OOXML）则是他专门为自己的Office全家桶设计的文档格式。我们熟悉的doc格式就是微软自己的专有格式，兼容性差。而docx则是在微软的游说和操弄下在ISO组织通过的规范。虽然如此，因为这是微软专门为自己设计的，docx的兼容性也比较差。这种文档格式充满了各种漏洞、错误和过去的特性。使用pandoc（一种自由的标记语言转换工具）就没办法处理doc文件。

OOXML的规范长达6000页，为其他人实施该规范制造了很大的障碍。OOXML使用了许多非标准的规范，造成与其他办公室软件（如LibreOffice）读取时发生不兼容或内容偏移的情形，目的是让Microsoft Office保持市场优势。比尔·盖茨在1998年留下的备忘录提到“不能让其他浏览器可完美显示MS Office文件，只能让自家公司的专属IE可正确显示”。OOXML可以说是微软延续了比尔·盖茨的精神，刻意让第三方软件无法很好地打开和编辑docx、xslx和pptx等文件。显然OOXML并非如其所声称的开放，也违背了标准化的初衷。文档规范并不是微软唯一一个滥用其垄断地位的地方。微软曾经故意让IE浏览器只支持部分网络规范。

开放文档格式如其名字OpenDocument Format（ODF），是一种开放的文档文件格式（open document format）。开放文档格式企图提供一个取代私有专利文档格式的一个方案，使得组织或个人不会因为文档格式而被厂商套牢（Vendor lock-in）。ODF的其中一个目标是保证用户能长期访问数据并不受技术及法律上的障碍。

丢掉:put_litter_in_its_place:MS Office全家桶，丢掉:put_litter_in_its_place:OOXML，现在开始就使用自由的办公套件以及开放的文档格式吧！
